...
function NotificationBell() {
    ...
    const fetchNotifications = async () => {
        ...
        try {
            const { data } = await axios.get(`https://eventro-backend.onrender.com/api/notifications`, config);
    ...
    const handleToggle = async () => {
        ...
        if (!isOpen && unreadCount > 0) {
            ...
            try {
                const config = { headers: { Authorization: `Bearer ${user.token}` } };
                await axios.put(`https://eventro-backend.onrender.com/api/notifications/read`, {}, config);
