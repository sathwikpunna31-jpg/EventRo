    ...
    <Link to={`/event/${event._id}`} className="event-card-link-wrapper">
      <div className='event-card'>
        ...
        <div className='card-content'>
          ...
          <p className='card-college'>{event.college?.name || event.college}</p>
          {event.club && <p className='card-club' style={{ fontSize: '0.8rem', color: 'var(--primary-color)', marginTop: '-5px', marginBottom: '10px' }}>{event.club?.name || event.club}</p>}
