...
function EventQnA({ event, eventId, onUpdate }) {
  ...
  const handleAsk = async (e) => {
    ...
    try {
      await axios.post(`http://localhost:5000/api/events/${eventId}/questions`, { question }, config);
      setQuestion('');
  ...
  const handleAnswer = async (questionId) => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/events/${eventId}/questions/${questionId}`, { answer }, config);
      setAnswer('');
