...
function CoordinatorDashboardPage() {
    ...
    const fetchDashboardData = useCallback(async (isInitial = false) => {
        ...
        try {
            ...
            const { data: clubs } = await axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/clubs`, config);
            ...
            if (myClub) {
                ...
                const { data: events } = await axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/events/myevents`, config);
                ...
                const { data: announcements } = await axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/announcements`, config);
