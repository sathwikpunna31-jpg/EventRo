...
function ManageDepartmentsPage() {
    ...
    const handleCreateDepartment = async (e) => {
        ...
        try {
            const config = { headers: { Authorization: `Bearer ${user.token}` } };
            await axios.post(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/departments`, {
                name: newDeptName,
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const config = { headers: { Authorization: `Bearer ${user.token}` } };
            await axios.delete(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/departments/${id}`, config);
    ...
    const handleUpdateDepartment = async (e, id) => {
        ...
        try {
            const config = { headers: { Authorization: `Bearer ${user.token}` } };
            await axios.put(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/departments/${id}`, {
                name: editDeptName,
